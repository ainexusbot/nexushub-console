...
/**
 * Inline tag editor for a single organization.
 * Reads the global tag pool from GET /organization-tags and toggles
 * assignments via POST /organizations/:id/tags and
 * DELETE /organizations/:id/tags/:tagId.
 */
export default function OrgTagSelector({ organizationId, tags = [], onChange, canEdit = true }) {
  ...
  const createTag = async () => {
    ...
    setError(null)
    try {
      const response = await api.post('/organization-tags', { name })
      if (!response.ok) {
        const data = await response.json().catch(() => ({}))
  ...
  return (
    <div className="relative flex flex-wrap items-center gap-1.5" ref={containerRef}>
      {tags.map((tag) => (
        <TagBadge key={tag.id} tag={tag} onRemove={canEdit ? removeTag : undefined} />
      ))}
